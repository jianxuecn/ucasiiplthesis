...
对于中文字体，模版已经预设好了大部分格式排版所对应的中文字体，例如章节标题为黑体、正文为宋体，配合英文的\textbf{加粗Text}使用宋体伪粗体（需要进一步改进）、
...
对参考文献的引用使用 \ltxcmdname{cite} 命令，其参数为bib文件中为每一篇参考文献定义的唯一的标识（关键字），即每一个参考文献项的第一个字段。本模版同样预定义了两个命令 \ltxcmdname{scite} 和 \ltxcmdname{upcite}，用于生成不同位置的文献引用标注，此处文献\citetns{wikibook2014latex}是句中引用（文献本身是句子成分之一），这是句末（上标，文献不是句子成分之一，仅表示名词或句子来自哪篇文献）引用\upcite{wikibook2014latex}。
